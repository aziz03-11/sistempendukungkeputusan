<?php
    ...
    if (isset($_POST['tambah_kandidat'])) {
        ...
        $kelas = mysqli_real_escape_string($dbcon, $_POST['kelas']);
        ...
        if(mysqli_num_rows($cek) > 0) {
            ...
            mysqli_query($dbcon, "INSERT INTO siswa (nisn, nama, kelas) VALUES ('$nisn', '$nama', '$kelas')");
    ...
    if (isset($_POST['edit_kandidat'])) {
        ...
        mysqli_query($dbcon, "UPDATE siswa SET nisn='$nisn_baru', nama='$nama', kelas='$kelas' WHERE nisn='$nisn_lama'");
...
<div class="container-fluid py-4">
    <div class="row mb-3 align-items-center">
        <div class="col-lg-6">
            <h2 class="fw-bold m-0 text-dark">Data Kandidat Siswa</h2>
            <p class="text-muted m-0">Kelola identitas, kelas, dan data peserta</p>
        </div>
            ...
            <table class="table table-hover table-borderless align-middle m-0">
                <thead class="table-light">
                    <tr>
                        ...
                        <th width="15%">NISN / ID</th>
                        <th>Nama Lengkap</th>
                        <th class="text-center">Kelas / Divisi</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center"><span class="badge bg-secondary rounded-pill px-3"><?= htmlspecialchars($row['kelas']) ?></span></td>
                        <td class="text-center">
                            <button class="btn btn-sm btn-outline-primary rounded-pill px-3 me-1" data-bs-toggle="modal" data-bs-target="#modalEdit<?= $row['nisn'] ?>">
                                <i class="fas fa-edit"></i> Edit
                            </button>
                            <a href="alternatif.php?hapus=<?= $row['nisn'] ?>" class="btn btn-sm btn-outline-danger rounded-pill px-3" onclick="return confirm('Yakin hapus data siswa ini?');">
                            ...
                            <div class="modal-content rounded-4 border-0 shadow">
                                ...
                                <form action="" method="POST">
                                    <div class="modal-body">
                                        ...
                                        <div class="mb-3">
                                            <label class="form-label text-muted small fw-bold">Kelas / Divisi</label>
                                            <input type="text" name="kelas" class="form-control rounded-3" value="<?= htmlspecialchars($row['kelas']) ?>" required>
                                        </div>
                                    </div>
                                    <div class="modal-footer border-top-0 pt-0">
                                        <button type="button" class="btn btn-light rounded-pill px-4" data-bs-dismiss="modal">Batal</button>
                                        <button type="submit" name="edit_kandidat" class="btn btn-primary rounded-pill px-4">Simpan</button>
                                    </div>
                                </form>
                            </div>
                        </div>
            ...
            <form action="" method="POST">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="kelas" class="form-control rounded-3" placeholder="Contoh: XII RPL 1" required>
                    </div>
                </div>
                <div class="modal-footer border-top-0 pt-0">
                    <button type="button" class="btn btn-light rounded-pill px-4" data-bs-dismiss="modal">Batal</button>
